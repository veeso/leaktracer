...
impl SymbolTable {
    pub fn new(size: usize, modules: &'static [&'static str]) -> Self {
        Self {
            modules,
    ...
    /// Iterates over the symbols in the table.
    pub fn iter(&self) -> impl Iterator<Item = (&&'static str, &Symbol)> {
        self.symbols.iter()
    }

    /// Gets a [`Symbol`] by its address.
    ...
    pub(crate) fn alloc(&mut self, bytes: usize) {
        ...
        // If the symbol does not exist, we create it with the given address and name.
...
mod test {
    ...
    fn test_should_allocate_symbol() {
        ...
        let address = demangle::get_demangled_symbol(&["leaktracer"]);
        let symbol = table.get(address).expect("Symbol should exist");
